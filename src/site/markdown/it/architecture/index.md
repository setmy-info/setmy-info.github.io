...
3. *Standardizing and stability as mush as possible*. So third partys (private companies etc) should have possibility to
   ...
   possibilites - component locations in folder structure, module/component folder structure, libraries and modules,
...
Environments should be separated and not used in moxed and crossed way (one environemnt komponent is usind another
environment komponent).
Peopel should not talk in mashine names, just with environmnet names.

### Standard profiles

Profiles for Maven, Spring, Micronaut etc profiles are per environment one to one. Name it as mentioned in Standard
environments list.
